...
Original file is located at
    https://colab.research.google.com/drive/1BkzyB1Bp3U-LLA65qQM6SrKcRC8OY8Q9
...
def solve_rank_one_update(L, U, b, u, v):
    ...
    """
    #Solve Ay=b
    yy = forward_substitution(L, b)
    yx = back_substitution(U, yy)
    #Solve Az=u
    uy = forward_substitution(L, u)
    ux = back_substitution(U, uy)
    x = yx + (np.matmul(v.T, yx)/(1-np.matmul(v.T, ux))) * ux
    return x
...
def apply_householder(v, u):
    ...
    array([-5. ,  1.6,  1.2])
    """
    return u - 2 * v * (np.matmul(v.T, u)/np.matmul(v.T, v))
...
def apply_householder_matrix(v, U):
    ...
    """
    vtU = np.matmul(v.T, U)
    cof = 2 / np.matmul(v.T, v)
    vtU = vtU * cof
    final = np.outer(vtU, v).T
    return U - final
...
mnist_digits = []
value, mnist_digits = np.unique(train_labels, return_counts=True)
...
mnist_r = np.linalg.norm(b_r - np.matmul(A_r, mnist_x))
...
test_image_0 = True
test_image_0_y = np.matmul(p, mnist_x)[0]
...
test_set = test_images[:, 4:24, 4:24].reshape([-1, 20*20])
test_image_y = np.matmul(test_set, mnist_x)
...
test_image_pred = []
for i in range(test_image_y.shape[0]):
    if test_image_y[i]>=0.5:
        test_image_pred.append(1)
    else:
        test_image_pred.append(0)
...
def mnist_classifiers():
    ...
    for i in range(10):
        B = np.copy(A)
        b = (train_labels == i).astype(np.float32)
        mnist_x = solve_qr_householder(B, b)
        xs.append(mnist_x)
    return np.stack(xs)
